...
wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/KusTa/koolshare_ss_rules_update/acelan_softcenter_ui/maintain_files/version1 > /tmp/version1
if [ "$?" == "0" ]; then
...
if [ "$ss_basic_gfwlist_update" == "1" ];then
	if [ ! -z "$version_gfwlist2" ];then
		if [ "$version_gfwlist1" != "$version_gfwlist2" ];then
			...
			wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/KusTa/koolshare_ss_rules_update/acelan_softcenter_ui/maintain_files/gfwlist.conf > /tmp/gfwlist.conf
...
if [ "$ss_basic_chnroute_update" == "1" ];then
	if [ ! -z "$version_chnroute2" ];then
		if [ "$version_chnroute1" != "$version_chnroute2" ];then
			...
			wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/KusTa/koolshare_ss_rules_update/acelan_softcenter_ui/maintain_files/chnroute.txt > /tmp/chnroute.txt
...
if [ "$ss_basic_cdn_update" == "1" ];then		
	if [ ! -z "$version_cdn2" ];then		
		if [ "$version_cdn1" != "$version_cdn2" ];then		
			...
			wget --no-check-certificate --timeout=8 -qO - https://raw.githubusercontent.com/KusTa/koolshare_ss_rules_update/acelan_softcenter_ui/maintain_files/cdn.txt > /tmp/cdn.txt		
